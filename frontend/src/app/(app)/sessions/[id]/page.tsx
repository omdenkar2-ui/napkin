...
import { ArrowLeft, CheckSquare, FileText, MessageSquare } from "lucide-react";
...
const SUMMARY_TEXT = "This session covers 142 feedback items from Slack and Intercom collected over the past week. The dominant pattern is mobile app performance, mentioned in 34% of feedback. Overall sentiment has improved by 12% compared to last week, driven by positive reception of the new dashboard redesign. Three critical issues require immediate attention.";
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-0px)]">
      ...
      <div className="h-14 border-b border-[#E5E2DC] flex items-center justify-between px-8 shrink-0 bg-[--background]">
        ...
        <button
          type="button"
          onClick={() => { console.log("create tasks"); toast.success("Tasks created"); }}
      ...
      <PageTransition className="flex flex-col lg:flex-row flex-1 min-h-0 p-4 md:p-8 gap-6">
        ...
        <div className="flex-[3] overflow-y-auto pr-2">
          ...
          <div className="pt-4 border-t border-[--border] mt-6">
            <button
              type="button"
              onClick={() => { console.log("create tasks for all"); toast.success("Tasks created for all patterns"); }}
              className="inline-flex items-center gap-2 h-9 px-4 bg-[--primary] text-[--primary-text] rounded-md text-sm font-medium hover:bg-[--primary-hover] transition-colors"
            >
              <CheckSquare className="w-4 h-4" />
        ...
        <div className="flex-[2] lg:border-l lg:border-[--border] lg:pl-6 border-t border-[--border] pt-6 lg:pt-0 lg:border-t-0 overflow-hidden flex flex-col">
          {/* Tab switcher */}
          <div className="flex items-center border-b border-[#E5E2DC] mb-4">
            <button
              type="button"
          ...
          {activeRightPanel === "evidence" ? (
            ...
            <SessionChat sessionId="a-1" dataPointCount={142} />
          )}
        </div>
      </PageTransition>
    </div>
