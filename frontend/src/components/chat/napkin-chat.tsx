    ...
    <div className="flex flex-col h-full">
            ...
            >
              ...
              >
                {msg.role === "user" ? (
                  ...
                  <>
                    <div className="prose prose-sm max-w-none [&_p]:my-1 [&_ul]:my-1 [&_ol]:my-1 [&_li]:my-0.5 [&_code]:bg-[--surface-hover] [&_code]:px-1 [&_code]:py-0.5 [&_code]:rounded [&_code]:text-[12px] [&_pre]:bg-[--surface-alt] [&_pre]:p-3 [&_pre]:rounded-lg [&_pre]:overflow-x-auto">
                      <ReactMarkdown>{msg.content}</ReactMarkdown>
                    </div>
                    {msg.data_summary && (msg.data_summary.sessions_searched > 0 || msg.data_summary.feedback_items_searched > 0) && (
                      <div className="flex items-center gap-1.5 mt-2 pt-2 border-t border-[--border]">
                        <Database className="w-3 h-3 text-[--text-disabled]" />
      ...
      <div className="border-t border-[--border] px-4 py-3">
        <form onSubmit={handleSubmit} className="flex items-end gap-2">
          <div className="flex-1 bg-[--surface] border border-[--border] rounded-xl px-3 py-2 focus-within:border-[--border-focus] transition-colors">
            <textarea
              ref={textareaRef}
              value={input}
              ...
              rows={1}
              className="w-full resize-none bg-transparent text-[13px] text-[--text-primary] placeholder:text-[--text-muted] outline-none max-h-[120px]"
            />
          </div>
          <button
            type="submit"
            disabled={!input.trim() || sendMutation.isPending}
            className="h-10 w-10 flex items-center justify-center rounded-xl bg-[--primary] text-[--primary-text] hover:bg-[--primary-hover] transition-colors disabled:opacity-30 disabled:pointer-events-none shrink-0"
          >
