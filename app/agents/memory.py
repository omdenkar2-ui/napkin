"""
Napkin — Decision Log & Memory Agent (Agent 9)
...
Uses in-memory storage for MVP. Supabase persistence comes in Prompt 10.
"""
...
async def store_decision(
    ...
    # Extract decision summary from spec
...
async def retrieve_context(
    ...
    Args:
        ...
        current_patterns: Optional list of current pain keywords for relevance ranking.
        memory_store: Optional override store (for testing). Uses module-level store if None.
    ...
    # Rank by relevance if current_patterns provided
    if current_patterns:
    ...
    # Accumulate constraints and non-goals across all decisions
    ...
    # Generate suggestions from past outcomes
    ...
    for r in records:
        ...
        if r.get("outcome_status") == "failed" and r.get("outcome_notes"):
            suggestions.append(
                f"Previous attempt '{r.get('spec_title', '')}' failed: {r['outcome_notes']}"
            )
